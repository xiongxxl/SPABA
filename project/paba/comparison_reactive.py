...
if __name__ == "__main__":
    ...
    statistics_detail = 'data/result/statistics_supervision/uspto_1000/find_alpha'
    statistics_folder=os.path.join(parent_dir,statistics_detail)
    df_criterion = pd.read_excel(os.path.join(statistics_folder, 'USPTO_50K_mark_1000_split_atoms_number.xlsx'))
    df_sample= pd.read_excel(os.path.join(statistics_folder, 'df_frag_smiles_multi_4_5.xlsx'))
    results = calculate_reactive_percentage(df_criterion, df_sample)
    results_df=pd.DataFrame(results)
    atoms_number_excel = f'frag_atoms_error_main_0.99.xlsx'  # save functional group path
