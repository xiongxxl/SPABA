...
def train_model(model, train_loader, val_loader, test_loader, criterion, optimizer, num_epochs, device, max_atom):
    model.to(device)
    train_losses, val_losses = [], []
    train_accs, val_accs = [], []         # 原先的 atom_error-based ACC
    ...
    for epoch in range(num_epochs):
        ...
        # Validation
        val_loss, val_acc, val_sacc, val_mcc = evaluate_model(model, val_loader, criterion, device, max_atom)
        val_losses.append(val_loss)
        val_accs.append(val_acc)
        ...
        print(f'Epoch {epoch + 1}/{num_epochs} - '
              f'Train Loss: {train_loss:.4f}, Train Acc(atom): {train_acc:.2f}%, '
              f'Train CAcc: {train_sacc:.2f}%, Train MCC: {train_mcc:.4f} | '
              f'Val Loss: {val_loss:.4f}, Val Acc(atom): {val_acc:.2f}%, '
    ...
    # Final evaluation on test set
    test_loss, test_acc, test_sacc, test_mcc = evaluate_model(model, test_loader, criterion, device, max_atom)
    print(f'\n Test Loss: {test_loss:.4f}, Test Acc(atom): {test_acc:.2f}%, '
    ...
    return (best_model_state_acc, best_model_state_mcc,best_model_state_sacc, best_val_acc,best_val_mcc,best_val_sacc, train_losses,
            val_losses,train_accs, val_accs, train_saccs, val_saccs, train_mccs, val_mccs,test_loss, test_acc, test_sacc, test_mcc)
