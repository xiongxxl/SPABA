...
def calculate_reactive_percentage(df_criterion, df_sample):
    ...
    df_criterion_axis = df_criterion[['reactant', 'paba_criterion_deep']]

    for index_criterion, row_criterion in df_criterion_axis.iterrows():

        value_smiles_criterion = row_criterion['reactant']
        try:
            value_location_criterion_tensor =torch.tensor(eval(row_criterion['paba_criterion_deep']))
...
### deal  uspto
    ...
    input_folder_name = f'smiles/data/result/statistics_reactive/compare/uspto/mcc_f1/uspto'
    input_sample_file = os.path.join(parent_dir, input_folder_name)
    label_excel = os.path.join(input_sample_file, 'USPTO_50K_mark_split_atoms_number_1098_rexgen_two_bond_deep.xlsx')
    df_criterion=pd.read_excel(label_excel)
    sample_excel= os.path.join(input_sample_file, 'USPTO_50K_mark_split_atoms_number_1098_rexgen_two_bond_deep.xlsx')
    df_sample=pd.read_excel(sample_excel)
    df_metrics_dict_single=calculate_reactive_percentage(df_criterion, df_sample)
    metrics_excel=os.path.join(input_sample_file, 'USPTO_50K_mark_split_atoms_number_1098_rexgen_two_bond_deep_evaluation.xlsx')
