...
# #     x = torch.randn(4, 512, 512)  # batch=4
# #     y = torch.randn(4, 512)
# #     out = model(x)
# #     loss = criterion(out, y)
# #     loss.backward()
# #     print("conv1 weight grad:", model.conv1.weight.grad.shape, model.conv1.weight.grad.mean().item())
# #     print("conv2 weight grad:", model.conv2.weight.grad.shape, model.conv2.weight.grad.mean().item())
# #     print("fc1 weight grad:", model.fc1.weight.grad.shape, model.fc1.weight.grad.mean().item())
# #     print("fc2 weight grad:", model.fc2.weight.grad.shape, model.fc2.weight.grad.mean().item())
# 
# 
# # ##network 2CNN+3FC
# import torch
...
#         self.pool = nn.MaxPool1d(2)
#
#         # After 3 convolutions + 2 pooling, sequence length goes from 512 -> 256 -> 128
#         flattened_size = hidden_size * (input_size // 4)
#
...
#         self.pool = nn.MaxPool1d(2)
#
#         # After 3 convolutions + 2 pooling, sequence length: 512 -> 256 -> 128
#         flattened_size = hidden_size * (input_size // 4)
#
#         # Four fully connected layers
#         self.fc1 = nn.Linear(flattened_size, hidden_size)
#         self.fc2 = nn.Linear(hidden_size, hidden_size)
#         self.fc3 = nn.Linear(hidden_size, hidden_size // 2)  # Optional: reduce dimension gradually
...
#         x = self.relu(self.fc2(x))  # -> [batch, hidden]
#         x = self.dropout(x)
#         x = self.relu(self.fc3(x))  # -> [batch, hidden//2]
#         x = self.dropout(x)
#         x = self.fc4(x)  # -> [batch, output_size]
#
#         return x
...
#         self.pool = nn.MaxPool1d(2)
#
#         # After 4 convolutions + 2 pooling (we'll pool after conv2 and conv4):
#         # Sequence length: 512 -> 256 (after pool1) -> 256 -> 128 (after pool2)
#         flattened_size = hidden_size * (input_size // 4)  # Same as before since we still do 2 pools
#
#         # Three fully connected layers
#         self.fc1 = nn.Linear(flattened_size, hidden_size)
#         self.fc2 = nn.Linear(hidden_size, hidden_size)
...
#         # Conv block 1
#         x = self.relu(self.conv1(x))  # -> [batch, hidden, 512]
#         x = self.relu(self.conv2(x))  # -> [batch, hidden, 512]
#         x = self.pool(x)             # -> [batch, hidden, 256]
#
#         # Conv block 2
#         x = self.relu(self.conv3(x))  # -> [batch, hidden, 256]
#         x = self.relu(self.conv4(x))  # -> [batch, hidden, 256]
#         x = self.pool(x)             # -> [batch, hidden, 128]
#
#         x = x.flatten(start_dim=1)   # -> [batch, hidden * 128]
#
